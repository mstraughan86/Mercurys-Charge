...
==========================================
Slack has a certain appeal and huge fan following in both developer and non-developer tech communities. It's slick user-interface, concept of teams and channels to keep communication separate and relevant, tons of integrations for productivity(Dropbox, Box, Google Calendar, Hangouts etc) and things like giphy and [reminders](https://get.slack.help/hc/en-us/articles/201259356-Using-slash-commands), make it fun to use. Also, their APIs facilitate developers to extend the functionalities and build a custom experience for their team.
...
This turorial aims to help developers get up and running with a simple node app that turns your slack channel into a custom command-line terminal. The app uses a helper module called [slack-terminalize](https://github.com/ggauravr/slack-terminalize)_(disclaimer: I developed it)_, that abstracts away the initial processing of the messages. It uses Slack's [Real-Time API Node client](https://github.com/slackhq/node-slack-client) and prepares a bot to listen and respond to your requests. 

Note that we won't be using [Slash Commands](https://api.slack.com/slash-commands) here, but instead interpret regular messages as commands. So if you were looking to learn about Slash Commands, this might not be the appropriate tutorial. May be my next one, may be!
...
From the list of dependencies in _package.json_, the only required dependency is _slack-terminalize_, but since the sample app has an example to show how to handle async commands, [request](https://www.npmjs.com/package/request) is used to make REST calls.

![Project Structure](project_structure.png "Project Strucrure")

#### config/
All the json files you might need for your app could go here. And I say 'could' because it's fairly flexible and can be changed to work with a different directory, with the right configuration parameters(more on that, later) passed. This is just one of the many ways to structure your app, but if you're new to Slack integrations, I suggest you stick with this.
      
#### commands.json
This is what makes adding custom commands a piece of cake. Each command is represented by a key-value pair: with key being the command name(I'll call it primary name), and value being a JSON with custom key-value pairs, that you would want to use for the command.

e.g: Here I use the following custom fields for each command:

- _alias_ - these are the aliases(let's call them secondary names) for the command, which can be used in the slack channel to invoke this command as well. It's best to keep the smallest name as the primary name and more meaningful, longer names as aliases.
...
Notice again that the key names in this file is same as the file names in _commands/_ directory.

### Code Walkthrough

Replace the _SLACK\_TOKEN_ in _index.js_ with the one for your bot. CONFIG\_DIR and COMMAND\_DIR are to tell _slack-terminalize_, where to look for config and command implementations respectively.
...
Login to your Slack team, either on web or app. The bot is added to #general channel by default, but you can invite the bot to any of the channels, even private ones, with the Slash Command: **_/invite @\<your-bot-name\>_**. Soon as you type _/invite @_, Slack should automatically suggest you the user names. If you don't see your bot listed there, go back and check if you have integrated the bot correctly.

![Invite Bot](invite_bot.png "Invite Bot")

Type _help_ or _halp_(alias, remember?) in the channel and voila, the bot should respond to your request. Go ahead and play around with _commands/help.js_ to change what you see in the response. As you can see from the implementation, this command just reads the _config/commands.json_ file to respond, so it's synchronous. Sometimes, you might need to do some async tasks, like quering a database or calling a rest endpoint, to fetch the response. Let's see how to go about that.

Take a peek at _commands/gem.js_ and you'll see that it remembers the channel(through an [IIFE](http://benalman.com/news/2010/11/immediately-invoked-function-expression/)) the message was posted in, and whenever it receives a response from the endpoint, posts back the massaged response into the same channel!
...
Try _gem ab_ in your Slack channel and you should see something like this:

![Gem Response](gem_response.png "Gem Response")

Again, try playing around with the formatting of response in _commands/gem.js_, to get the hang of it. So, now you have a bot listening on invited channels, responding to your requests. Let's see how you can add custom commands.

### Adding Custom Command Implementations
Add your new command in _config/commands.json_: as mentioned before, the key will be the primary command name. Aliases for the command go in as an array of values in _alias_, as shown below.
...
Currently, command names with space in them are **not** supported. Create a file with the same name as the primary name of your command above(in this case, _your-command-name.js_) in _commands/_ directory. Assign _module.exports_ to the command implementation function, as shown below.
...
Refer [documentation](https://github.com/slackhq/node-slack-client) of node-slack-client, for more on _User_ and _Channel_ objects.
...
_slack-terminalize_ module takes two parameters, an options object and a config object.
...
- Fork the projects: [_slack-terminalize_]() and [its sample app](). Play around, contribute and help improve it.
- Write to me about how you're using Slack for productivity, or if you have any suugestions on how this can be improved. I'm all ears to learn the creative applications of the power bestowed upon developers by Slack API
